...
class DeviceModelTest(TestCase):
    def setUp(self):
        ...
        self.device = Device.objects.create(name='test device', owner_user=self.user, owner_hospital=self.hospital, owner_ward=self.ward)
...
class VitalDataModelTest(TestCase):
    def setUp(self):
        ...
        self.vital_data=VitalData.objects.create(threshold='100', can_override_threshold=False, patient=self.patient, vital_type='heart beat', value='105', timestamp=timezone.now(), device=self.device)
